...
for i in range(0,len(data)):
    ...
    if data[i]<0:
        data[i] =-data[i]
    order = int((data[i]-start_place)//((end_place-start_place)/num))
    stack[order] = stack[order]+1
...
plt.title('Stack Plot(wavelength distribution of green laser)',fontsize=80)
plt.scatter(max_index, stack[index],c = 'r')
plt.ylim(-0.5,stack[index]*1.1)
plt.xlim(400,600)
